...
const NicotineStatistics = () => {
  ...
  useEffect(() => {
    ...
    async function startFetching() {
      const json = await cigaretteService.getBrandNicotineStatistics(page, pageSize);
      if (!ignore) {
        setBrands(json);
      }
    }
        ...
        {brands.map((_, index) => (
          <Grid item xs={1} sm={3} lg={2} key={index}>
            <BrandCard brand={brands[index]}>
                <br />
                Average Nicotine: {brands[index].averageNicotine}
            </BrandCard>
          </Grid>
        ))}
      </Grid>
...
export default NicotineStatistics;
